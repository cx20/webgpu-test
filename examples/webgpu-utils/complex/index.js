...
// gl-matrix is loaded as a global via a <script> tag (see index.html)
...
// Resolve a glTF image to a URL (external uri or an object URL for an embedded bufferView)
function resolveGLTFImageURL(gltf, buffers, baseUrl, textureIndex) {
...
// Load a glTF texture through webgpu-utils (handles fetch + bitmap + mips)
async function loadTextureFromGLTF(device, gltf, buffers, baseUrl, textureIndex) {
    const url = resolveGLTFImageURL(gltf, buffers, baseUrl, textureIndex);
    return createTextureFromImage(device, url, { mips: true, flipY: false });
}

// Load 6 images into a cube texture through webgpu-utils
async function loadCubeMap(device, urls) {
    return createTextureFromImages(device, urls, { viewDimension: 'cube' });
}
...
async function main() {
    ...
    // Manage the uniform blocks with webgpu-utils' structured views
    // (reused across every draw; queue.writeBuffer snapshots the data each call)
    const mainUniformValues = makeStructuredView(makeShaderDataDefinitions(vsSource).uniforms.uniforms);
    const skyboxUniformValues = makeStructuredView(makeShaderDataDefinitions(vsSkyboxSource).uniforms.uniforms);
    ...
    function render() {
        ...
        skyboxUniformValues.set({
            projectionMatrix,
            viewMatrix: skyboxView,
        });
        device.queue.writeBuffer(skyboxUniformBuffer, 0, skyboxUniformValues.arrayBuffer);
        ...
        for (const ground of groundTracks) {
            ...
            mainUniformValues.set({
                modelMatrix,
                viewMatrix,
                projectionMatrix,
                normalMatrix,
                lightDir: [1, 1, 1, 0],
                baseColor: ground.track.color,
                flags: [0, 0, 1, 0], // hasSkinning, hasTexture, hasNormals, -
            });
            device.queue.writeBuffer(ground.uniformBuffer, 0, mainUniformValues.arrayBuffer);
                ...
                if (node.meshIndex !== undefined) {
                    ...
                    for (let primIdx = 0; primIdx < mesh.primitives.length; primIdx++) {
                        ...
                        mainUniformValues.set({
                            ...
                            flags: [
                                prim.hasSkinning && nodeSkin ? 1 : 0, // hasSkinning
                                prim.hasTexture ? 1 : 0,              // hasTexture
                                prim.hasNormals ? 1 : 0,              // hasNormals
                                0,
                            ],
                        });
                        device.queue.writeBuffer(instance.uniformBuffer, 0, mainUniformValues.arrayBuffer);
