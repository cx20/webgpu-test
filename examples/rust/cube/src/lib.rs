...
async fn run() {
    ...
    // Replicate each face color over its four vertices.
    let mut colors = [[0.0f32; 4]; 24];
    for (face, color) in face_colors.iter().enumerate() {
        for v in 0..4 {
            colors[face * 4 + v] = *color;
        }
    }
    let indices: [u32; 36] = [
    ...
    let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        label: None,
        // `None` requests an automatically derived pipeline layout from the shader.
        layout: None,
