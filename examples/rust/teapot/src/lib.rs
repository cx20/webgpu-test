...
// Model and texture embedded at compile time so the demo needs no extra fetch.
// Teapot model from: https://github.com/gpjt/webgl-lessons/blob/master/lesson14/Teapot.json
const TEAPOT_JSON: &str = include_str!("../../../../assets/json/teapot.json");
// Texture from: https://github.com/gpjt/webgl-lessons/blob/master/lesson14/
const TEXTURE_BYTES: &[u8] =
    include_bytes!("../../../../assets/textures/arroway.de_metal+structure+06_d100_flat.jpg");

#[derive(Deserialize)]
struct TeapotModel {
    #[serde(rename = "vertexPositions")]
    vertex_positions: Vec<f32>,
    #[serde(rename = "vertexNormals")]
    vertex_normals: Vec<f32>,
    #[serde(rename = "vertexTextureCoords")]
...
async fn run() {
    ...
    let model: TeapotModel = serde_json::from_str(TEAPOT_JSON).expect("failed to parse teapot.json");

    let vertex_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Vertex Buffer"),
        contents: bytemuck::cast_slice(&model.vertex_positions),
        usage: wgpu::BufferUsages::VERTEX,
    });
    let normal_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Normal Buffer"),
        contents: bytemuck::cast_slice(&model.vertex_normals),
        usage: wgpu::BufferUsages::VERTEX,
    });
    let coord_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("TexCoord Buffer"),
        contents: bytemuck::cast_slice(&model.vertex_texture_coords),
        usage: wgpu::BufferUsages::VERTEX,
    });
    ...
    // Point light location (x, y, z) padded to a 16-byte uniform.
    let light_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Light Buffer"),
        contents: bytemuck::cast_slice(&[100.0f32, 0.0, 100.0, 0.0]),
    ...
    *g.borrow_mut() = Some(Closure::<dyn FnMut(f64)>::new(move |timestamp: f64| {
        ...
        let view = Mat4::from_translation(Vec3::new(0.0, 0.0, -35.0))
            * Mat4::from_axis_angle(Vec3::Y, rad);
            ...
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        {
            ...
            rpass.set_vertex_buffer(2, coord_buf.slice(..));
