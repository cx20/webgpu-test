...
const TEXTURE_BYTES: &[u8] = include_bytes!("../../../../assets/textures/earth.jpg");
...
async fn run() {
    ...
    // 3x3 grid of primitives. Each entry: (geometry, grid position, extra X rotation).
    let s = 2.5;
    let specs: [(Geometry, [f32; 3], f32); 9] = [
        // Row 1
        (geometry::plane(1.5, 1.5, 4), [-s, s, 0.0], 0.0),
        (geometry::cube(1.2), [0.0, s, 0.0], 0.0),
        (geometry::sphere(0.8, 48, 24), [s, s, 0.0], 0.0),
        // Row 2
        (geometry::circle(0.8, 32), [-s, 0.0, 0.0], -std::f32::consts::FRAC_PI_2),
        (geometry::cylinder(1.2, 0.6, 0.6, 32), [0.0, 0.0, 0.0], 0.0),
        (geometry::cylinder(1.2, 0.7, 0.0, 32), [s, 0.0, 0.0], 0.0),
        // Row 3
        (geometry::tetrahedron(0.8), [-s, -s, 0.0], 0.0),
        (geometry::octahedron(0.8), [0.0, -s, 0.0], 0.0),
        (geometry::torus(0.5, 0.25, 32, 16), [s, -s, 0.0], 0.0),
    ];

    let meshes: Vec<Mesh> = specs
        .into_iter()
        .map(|(geo, position, rotate_x)| {
            let position_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Position Buffer"),
                contents: bytemuck::cast_slice(&geo.positions),
                usage: wgpu::BufferUsages::VERTEX,
            });
            let uv_buf = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("UV Buffer"),
                contents: bytemuck::cast_slice(&geo.uvs),
                usage: wgpu::BufferUsages::VERTEX,
            });
    ...
    *g.borrow_mut() = Some(Closure::<dyn FnMut(f64)>::new(move |timestamp: f64| {
        ...
        for mesh in &meshes {
            let model = Mat4::from_translation(mesh.position)
                * Mat4::from_rotation_y(time * 0.5)
                * Mat4::from_rotation_x(mesh.rotate_x);
            let mvp = projection * view * model;
            queue.write_buffer(&mesh.uniform_buf, 0, bytemuck::cast_slice(&mvp.to_cols_array()));
        }
            ...
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        {
            ...
            for mesh in &meshes {
                ...
                rpass.set_vertex_buffer(0, mesh.position_buf.slice(..));
                rpass.set_vertex_buffer(1, mesh.uv_buf.slice(..));
