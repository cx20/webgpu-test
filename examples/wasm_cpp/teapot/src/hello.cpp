...
WGPUBuffer normalBuf;  // normal buffer
...
void createPipelineAndBuffers() {
	WGPUShaderModule vertMod = createShader(teapot_vert_wgsl);
	WGPUShaderModule fragMod = createShader(teapot_frag_wgsl);
	...
	WGPUVertexAttribute normalAttr = {};
	...
	coordAttr.offset = 0;
	coordAttr.shaderLocation = 2;

	WGPUVertexBufferLayout vertexBufferLayouts[3] = {};
	...
	vertexBufferLayouts[1].attributes = &normalAttr;
	vertexBufferLayouts[2].arrayStride = 2 * sizeof(float);
	vertexBufferLayouts[2].attributeCount = 1;
	vertexBufferLayouts[2].attributes = &coordAttr;
	...
	// Teapot model data (from teapot.h)
	vertBuf   = createBuffer(teapot_positions, teapot_positions_count * sizeof(float), WGPUBufferUsage_Vertex);
	normalBuf = createBuffer(teapot_normals,   teapot_normals_count   * sizeof(float), WGPUBufferUsage_Vertex);
	coordBuf  = createBuffer(teapot_texcoords, teapot_texcoords_count * sizeof(float), WGPUBufferUsage_Vertex);
	indxBuf   = createBuffer(teapot_indices,   teapot_indices_count   * sizeof(uint32_t), WGPUBufferUsage_Index);
	...
	// Light uniform buffer (vec3, padded to 16 bytes)
	WGPUBufferDescriptor lightDesc = {};
	lightDesc.usage = WGPUBufferUsage_Uniform | WGPUBufferUsage_CopyDst;
	lightDesc.size  = 4 * sizeof(float);
	lightBuf = wgpuDeviceCreateBuffer(device, &lightDesc);
	float light[4] = { 100.0f, 0.0f, 100.0f, 0.0f };
	wgpuQueueWriteBuffer(queue, lightBuf, 0, light, sizeof(light));

	// Texture and sampler
	WGPUTexture texture = createTextureFromMemory(metal_jpg, metal_jpg_len);
	...
	mat4_perspective(projection, 45.0f, (float)curW / (float)curH, 0.1f, 1000.0f);
}
...
bool redraw() {
	...
	float view[16];
	mat4_identity(view);
	mat4_translate(view, 0.0f, 0.0f, -35.0f);
	float rad = (float)(emscripten_get_now() / 1000.0);
	mat4_rotate(view, rad, 0.0f, 1.0f, 0.0f);
	...
	wgpuRenderPassEncoderSetVertexBuffer(pass, 1, normalBuf, 0, WGPU_WHOLE_SIZE);
	wgpuRenderPassEncoderSetVertexBuffer(pass, 2, coordBuf, 0, WGPU_WHOLE_SIZE);
	wgpuRenderPassEncoderSetIndexBuffer(pass, indxBuf, WGPUIndexFormat_Uint32, 0, WGPU_WHOLE_SIZE);
	wgpuRenderPassEncoderDrawIndexed(pass, teapot_indices_count, 1, 0, 0, 0);
