...
WGPUBuffer colBuf;   // color buffer
...
void createPipelineAndBuffers() {
	WGPUShaderModule vertMod = createShader(cube_vert_wgsl);
	WGPUShaderModule fragMod = createShader(cube_frag_wgsl);
	...
	WGPUVertexAttribute colAttr = {};
	colAttr.format = WGPUVertexFormat_Float32x4;
	colAttr.offset = 0;
	colAttr.shaderLocation = 1;
	...
	vertexBufferLayouts[1].attributes = &colAttr;
	...
	for (int face = 0; face < 6; face++) {
		for (int v = 0; v < 4; v++) {
			for (int k = 0; k < 4; k++) {
				colors[(face * 4 + v) * 4 + k] = faceColors[face][k];
			}
		}
	}
	...
	colBuf  = createBuffer(colors,    sizeof(colors),    WGPUBufferUsage_Vertex);
	indxBuf = createBuffer(indices,   sizeof(indices),   WGPUBufferUsage_Index);

	// Uniform buffer (4x4 matrix) and bind group
...
bool redraw() {
	...
	wgpuRenderPassEncoderSetVertexBuffer(pass, 1, colBuf, 0, WGPU_WHOLE_SIZE);
